...
function PlanningContent() {
  ...
  useEffect(() => {
    async function fetchPosts() {
      ...
      // Query per post attivi (default)
      // archived_at IS NULL AND visible_until > now
      ...
      if (archivedError) {
        console.error('Error fetching archived posts:', archivedError)
      }

      const formatPost = (post: Record<string, unknown>): Post => ({
        id: post.id as string,
        pillar: (post.pillar as string) || '',
        platform: (post.platform as string) || 'facebook',
        scheduledDate: post.scheduled_date as string,
          ...
          for (const post of postsWithThumbs) {
            if (post.imageThumbPath) {
              const { data } = await supabase.storage
                .from('post-images')
                .createSignedUrl(post.imageThumbPath, 3600)
              if (data?.signedUrl) {
      ...
      if (archivedData) {
        const formattedArchived = archivedData
          .filter(post => post.copy)
          .map(formatPost)
        setArchivedPosts(formattedArchived)
      }
  ...
  const handleReplicate = async (post: Post, targetPlatform: string) => {
    ...
    // Calcola visible_until per nuovo post (created_at + 60 giorni)
    const createdAt = new Date()
    const visibleUntil = new Date(createdAt)
  ...
  const displayPosts = showArchive ? archivedPosts : posts
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-4xl mx-auto py-8">
        ...
        {!hasPillars(profile) && !showArchive && (
          <Card className="mb-4">
        ...
        {showArchive && (
          <Card className="mb-4 bg-gray-100">
            <p className="text-xs text-gray-500 text-center">
              Post archiviati (oltre 60 giorni). Solo lettura, senza immagini.
            </p>
          </Card>
        )}
        ...
        <div className="space-y-3">
          {displayPosts.map((post) => {
            ...
            const cardBg = showArchive ? 'bg-gray-50' : (platformColors[post.platform] || '')
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  ...
                  {isCopied && (
                    <div className="flex-shrink-0 ml-2 flex flex-col items-center justify-center min-h-[48px]">
                ...
                {/* Azioni: Replica + Copia + WhatsApp (solo per post attivi) */}
                {!showArchive && (
